...
appendScript("navigator/socket.io.js");
...
const socket = io(serversckio); // Remplacez l'URL par l'adresse de votre serveur



function requestServerTimer(){
    var url = "https://192.168.1.222:13000/index.py";
    var data = {
        starttime: now, 
...
function onGetSpeak_Gemini(speak){
    if(isGeminiCalled){
		...
		setTimeout(function(){
			document.execCommand("insertText", true, speak[0]);
            speakVoice(speak[0]+". Dites envoyer pour continuer, ou gemini pour une autre question.");
		}, 2000);
    }else{
        for(var i in speak){
            if(["gemini","jenny","les mini","janine"].indexOf(speak[i]) !== -1){
                speakVoice("Je vous écoute");
                isGeminiCalled = true;
            }
            if(["envoyer","voyez"].indexOf(speak[i]) !== -1){
                $(".send-button").click();
                timerReadChat();
            }
        }
